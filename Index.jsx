...
const ConfirmModal = ({ isOpen, onClose, onConfirm, title, message, confirmText = "Sí, Eliminar", confirmColor = "bg-red-500 hover:bg-red-600" }: any) => {
...
export default function App() {
  ...
  const [view, setView] = useState<'dashboard' | 'editor' | 'live'>('dashboard');
  ...
  const [deleteModal, setDeleteModal] = useState<{isOpen: boolean, raffleId: string | null}>({ isOpen: false, raffleId: null });
  const [resetModal, setResetModal] = useState<{isOpen: boolean, raffleId: string | null}>({ isOpen: false, raffleId: null });
  ...
  // Estados Live
  const [liveStep, setLiveStep] = useState<'ready' | 'countdown' | 'results'>('ready');
  ...
  useEffect(() => {
    ...
    const savedBgType = localStorage.getItem('elite-bg-type') as any;
  ...
  const stopSound = (audioRef: any) => { if (audioRef.current) { audioRef.current.pause(); audioRef.current.currentTime = 0; } };
  ...
  const requestReset = (id: string, e?: React.MouseEvent) => {
      if (e) { e.stopPropagation(); e.preventDefault(); }
  ...
  const duplicateRaffle = (raffle: any, e: React.MouseEvent) => {
      e.stopPropagation();
  ...
  const handleMainAction = (raffle: any, e: React.MouseEvent) => {
      e.stopPropagation();
      loadRaffleIntoEditor(raffle);
      setView('live');
      
      if (raffle.status === 'completed') {
          // Si ya terminó, mostrar resultados directamente
          setWinners(raffle.results);
          setLiveStep('results');
          setCurrentWinnerIndex(0);
          setShowConfetti(true); // Opcional: Celebrar al ver resultados
          setTimeout(() => playSound(applauseAudio), 500);
      } else {
          // Si es nuevo, ir a 'Ready'
          setLiveStep('ready');
          setWinners([]);
      }
  };
  ...
  const getWinnerFontSize = (text: string) => {
    if (!text) return "text-5xl md:text-8xl";
    const len = text.length;
    if (len < 10) return "text-5xl md:text-8xl"; // Muy corto (ej: "Juan")
    if (len < 20) return "text-4xl md:text-6xl"; // Medio (ej: "Juan Perez")
    if (len < 35) return "text-3xl md:text-5xl"; // Largo
    return "text-2xl md:text-4xl"; // Muy largo
  };
  ...
  }, [view, liveStep, countdown, participants, winners, currentRaffleId]); // Añadido dependencias necesarias

  const triggerCelebration = () => { setShowConfetti(true); setTimeout(() => playSound(applauseAudio), 100); };
  
  const resetLive = () => {
      // Solo permite reiniciar visualmente si estás en modo live, pero no borra los datos guardados
      // Para borrar datos usar el botón "Reset" del dashboard
      playSound(clickAudio);
  ...
  // --- Funciones Archivos ---
  const handleBgUpload = (e: any) => { /* ... igual ... */ 
    const file = e.target.files?.[0]; if (file) { const reader = new FileReader(); reader.onload = (ev: any) => { try { localStorage.setItem('elite-bg', ev.target.result); localStorage.setItem('elite-bg-type', file.type.startsWith('video/') ? 'video' : 'image'); } catch(e) {} setBgSource(ev.target.result); setBgType(file.type.startsWith('video/') ? 'video' : 'image'); showToast("Fondo actualizado"); }; reader.readAsDataURL(file); }
  };
  const handleLogoUpload = (e: any) => { /* ... igual ... */
    const file = e.target.files?.[0]; if (file) { const reader = new FileReader(); reader.onload = (ev: any) => { try { localStorage.setItem('elite-logo', ev.target.result); } catch(e) {} setLogo(ev.target.result); showToast("Logo actualizado"); }; reader.readAsDataURL(file); }
  };
  const handleFileUpload = (e: any) => { /* ... igual ... */
    const file = e.target.files?.[0]; if (file) { const reader = new FileReader(); reader.onload = (ev: any) => { const rawNames = ev.target.result.split(/[\r\n,]+/).map((n: string) => n.trim()).filter((n: string) => n !== ''); const unique = Array.from(new Set([...participants, ...rawNames])); setParticipants(unique); showToast(`Cargados ${unique.length} participantes`); }; reader.readAsText(file); }
  };
                            ...
                            {raffle.status === 'completed' ? (
                                <div className="bg-black/30 rounded-lg p-2 text-xs text-white/70 h-full overflow-hidden">
                                    ...
                                    <div className="flex flex-col gap-1 overflow-y-auto max-h-[60px] custom-scrollbar">
                                        {raffle.results?.map((w: string, i: number) => (
